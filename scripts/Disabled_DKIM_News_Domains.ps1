param ($UserName, $Password)
   ...
   $password =  ConvertTo-SecureString -String $Password
    ...
    Try
    {
        ...
        Write-host "Disabled DKIM Domain for  $User :" -ForegroundColor Yellow 
		...
		if($domain_name -notlike '*onmicrosoft.com') {
			...
			Try
				{
				Write-host "Disabled DKIM Domain For $($domain_name):" -ForegroundColor Yellow -NoNewline
				
				 $rr=Set-DkimSigningConfig -Identity $domain_name -Enabled $false -ErrorAction stop
